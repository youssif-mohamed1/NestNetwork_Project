...
class Login(UserMixin,db.Model):
    ...
    gender=db.Column(db.String(50))
    ph_num=db.Column(db.String(50))
>>>>>>> da6539356ecf073abdd09bd25d0857c6bd876fd8
...
def forgot_password(x_email):
    ...
    if user_email:
        ...
        # sending the message
        email_alert("Verification Code",
                    f"Hello {user_name}, This the Verification Code: {rand_code}", "moustafaalaa30@gmail.com")
        done = True
...
def generate_random_pass():
    ...
    # is an built-in variable in function choice(left-click+ctrl: to see)
...
def email_alert(subject, body, to):
    ...
    # (Secure Socket Layer/Transport Layer Security), provide secure connection
    server.starttls()
...
def communities():  # main-page
    # Loading the HTML page
    return render_template("communities.html", pagetitle="Homepage")


@app.route("/myaccount", methods=['POST', 'GET'])
def myaccount():  # main-page
    # Loading the HTML page
    return render_template("myaccount.html", pagetitle="myaccount")


@app.route("/home", methods=['POST', 'GET'])
def home():  # main-page
    # Loading the HTML page
    return render_template("home.html", pagetitle="Homepage", logged="logged-no")


@app.route("/home_loggedin", methods=['POST', 'GET'])
def home_loggedin():  # main-page
    # Loading the HTML page
    return render_template("home_loggedin.html", pagetitle="Homepage", logged="logged-no")
...
# --
        ...
        while (gen_id):
            ...
            gen_id = Stud.query.filter_by(stud_id=ruser_id).first() or Prof.query.filter_by(
                prof_id=ruser_id).first()  # authinticate if the email entered already exist
...
def confirm_vcode():
    ...
    if vref_code == str(rand_code_global):
        ...
        update_user(user_id=id_global,
                    ...
                    ph_num="")
        email_alert(
            "Password Reset", f"Hello {name_global}, This is your new password: {rand_pass}", "moustafaalaa30@gmail.com")
        print("Password Sent")
...
def handle_resend():
    ...
    email_alert("Verification Code",
                f"Hello {name_global}, This the Verification Code: {rand_code_global}", "moustafaalaa30@gmail.com")
...
def vref():
    ...
    if rend_page == "":
        rend_page = "vref"
    if rend_page != "vref":
        return redirect(url_for("confirm_vcode"))
